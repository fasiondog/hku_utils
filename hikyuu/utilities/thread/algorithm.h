...
// --- 分支 B: 无返回值 (T == void) ---
template <typename Executor, typename Func>
auto co_dispatch_impl(Executor exec, Func&& func, std::false_type) 
  -> dispatch_task<void> {
 ...
 *           if (!cached_data || cached_data->wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
 ...
 */
template<>
inline auto await_future<void>(std::shared_ptr<std::future<void>> fut_ptr) -> asio::awaitable<void> {
    auto exec = co_await asio::this_coro::executor;
    asio::steady_timer timer(exec);
