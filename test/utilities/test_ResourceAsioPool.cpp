...
TEST_CASE("test_ResourceAsioPool_multithreaded_executor_stress") {
    boost::asio::io_context io_ctx;
    Parameter param;
    ResourceAsioPool<TestResource> pool(io_ctx, param, 5, 20);  // 最大 5 个活动资源，最多 20 个总资源
        ...
        co_spawn(io_ctx, [&]() -> boost::asio::awaitable<void> {
            try {
                ...
                while (current_count > static_cast<size_t>(expected)) {
                    max_concurrent_observed.compare_exchange_weak(expected, static_cast<int>(current_count));
                }

                // 验证活动资源数不超过最大值限制
                REQUIRE(pool.count() <= 5);
        ...
        co_spawn(io_ctx, [&]() -> boost::asio::awaitable<void> {
            try {
                ...
                while (current_count > static_cast<size_t>(old_value)) {
                    max_count_observed.compare_exchange_weak(old_value, static_cast<int>(current_count));
                }
