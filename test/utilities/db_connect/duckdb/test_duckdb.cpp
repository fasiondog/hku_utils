...
TEST_CASE("test_DuckDB_optimized_batch_operations") {
    Parameter param;
    param.set<std::string>("db", "test_optimized_batch.duckdb");

    DuckDBConnectPtr driver = std::make_shared<DuckDBConnect>(param);
    REQUIRE(driver);

    // 创建测试表 - 显式指定id值避免自增约束问题
    driver->exec("DROP TABLE IF EXISTS optimized_batch_test");
    driver->exec(
      "CREATE TABLE optimized_batch_test (id INTEGER PRIMARY KEY, name VARCHAR(50), value DOUBLE)");

    // 定义测试数据结构
    struct OptimizedBatchData {
        int record_id;  // 改名为record_id避免冲突
        std::string name;
        double value;
        ...
        static std::string getTableName() {
            return "optimized_batch_test";
        }
        static const char* getInsertSQL() {
            return "INSERT INTO optimized_batch_test (id, name, value) VALUES (?, ?, ?)";
        }
        static const char* getUpdateSQL() {
            return "UPDATE optimized_batch_test SET name=?, value=? WHERE id=?";
        }

        void save(const SQLStatementPtr& st) const {
            st->bind(0, static_cast<int64_t>(record_id));
            st->bind(1, name);
            st->bind(2, value);
        }
        ...
        void update(const SQLStatementPtr& st) const {
            st->bind(0, name);
            st->bind(1, value);
            st->bind(2, static_cast<int64_t>(record_id));
        }
    };

    // 生成测试数据 - 显式设置id值
    std::vector<OptimizedBatchData> test_data;
    for (int i = 1; i <= 1000; ++i) {
        OptimizedBatchData item;
        item.record_id = i;
        item.name = fmt::format("Test Item {}", i);
        item.value = i * 1.5;
        test_data.push_back(item);
    ...
    // 验证数据插入成功
    auto count = driver->queryNumber<int64_t>("SELECT COUNT(*) FROM optimized_batch_test");
    CHECK(count == 1000);

    // 测试批量更新
    for (auto& item : test_data) {
        item.value += 100;  // 修改值用于更新测试
    }

    start_time = std::chrono::high_resolution_clock::now();
    driver->batchUpdate(test_data, true);
    end_time = std::chrono::high_resolution_clock::now();

    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
    std::cout << "Batch update took: " << duration.count() << " ms" << std::endl;

    // 验证更新成功
    auto avg_value = driver->queryNumber<double>("SELECT AVG(value) FROM optimized_batch_test");
    CHECK(avg_value > 100.0);

    std::cout << "Optimized batch operations test completed successfully!" << std::endl;
}
...
/** @par 检测终点 */
TEST_CASE("test_duckdb_end") {
    std::cout << "end test duckdb" << std::endl;
}