...
TEST_CASE("test_co_dispatch_void") {
    ...
    auto task = co_dispatch(pool.executor(), [&executed]() {
...
TEST_CASE("test_co_dispatch_fire_and_forget") {
    ...
    for (int i = 0; i < 10; ++i) {
        co_dispatch(pool.executor(), [&counter]() {
...
TEST_CASE("test_co_dispatch_lambda_capture") {
    ...
    auto task = co_dispatch(pool.executor(), [value1, value2 = std::move(value2)]() 
...
TEST_CASE("test_co_dispatch_string_return") {
    ...
    auto task = co_dispatch(pool.executor(), [expected]() -> std::string {
        return expected;
    });
...
TEST_CASE("test_co_dispatch_complex_type") {
    ...
    auto task = co_dispatch(pool.executor(), []() -> TestData {
        return TestData{42, "test", 3.14};
    });
...
TEST_CASE("test_co_dispatch_move_only_type") {
    ...
    auto task = co_dispatch(pool.executor(), []() -> std::unique_ptr<int> {
...
TEST_CASE("test_co_dispatch_multiple_executors") {
    ...
    auto t1 = co_dispatch(pool1.executor(), [&counter]() {
        counter.fetch_add(1, std::memory_order_relaxed);
    });
    
    auto t2 = co_dispatch(pool2.executor(), [&counter]() {
        counter.fetch_add(1, std::memory_order_relaxed);
    });
...
TEST_CASE("test_co_dispatch_no_wait_basic") {
    ...
    // 提交任务但不等待
    co_dispatch_no_wait(pool.executor(), [&counter]() {
        counter.fetch_add(1, std::memory_order_relaxed);
    });
...
TEST_CASE("test_co_dispatch_no_wait_void") {
    ...
    co_dispatch_no_wait(pool.executor(), [&executed]() {
        executed.store(true, std::memory_order_relaxed);
    });
...
TEST_CASE("test_co_dispatch_no_wait_multiple_executors") {
    ...
    co_dispatch_no_wait(pool1.executor(), [&counter]() {
        counter.fetch_add(1, std::memory_order_relaxed);
    });
    
    co_dispatch_no_wait(pool2.executor(), [&counter]() {
        counter.fetch_add(1, std::memory_order_relaxed);
    });
